...
def inference_top_down_pose_model(model,
    ...
    if bbox_thr is not None:
        # if the value of bbox shape is not "5", the assertion will occur.
        assert bboxes.shape[1] == 5, f"the bbox size is expected 5, but got {bboxes.shape[1]}"
...
def inference_bottom_up_pose_model(model,
    ...
    test_pipeline = [PoseLoadImage(channel_order=channel_order)
                     ] + cfg.test_pipeline[1:]
...
def _inference_single_pose_model(model,
    ...
    # cfg.data_cfg['image_data'] = bboxes[5] # list
    ...
    if next(model.parameters()).is_cuda:
        ...
        batch_data['img'] = batch_data['img'].to(device) # image tensor data
    ...
    # forward the model
    # mmpose->models->detectors->top_down->TopDown class forward
    with torch.no_grad():
        start.record()
        result = model(
            img=batch_data['img'],
